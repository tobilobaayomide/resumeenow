...
const AuthModal: React.FC<AuthModalProps> = ({ open, onClose, mode }) => {
  ...
  const handleOAuthLogin = async (provider: OAuthProvider) => {
    ...
    try {
      ...
      const providerLabel = OAUTH_PROVIDERS.find((item) => item.provider === provider)?.label ?? provider;
      const message = err instanceof Error ? err.message : `Failed to login with ${providerLabel}`;
      toast.error(message);
    } finally {
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
      ...
      >
        <button
          ...
          className="absolute top-4 right-4 p-2 cursor-pointer rounded-full text-gray-400 hover:text-gray-900 transition-all duration-200 z-10 disabled:opacity-50"
          onClick={closeModal}
          disabled={isBusy}
        >
          <FiX size={20} />
