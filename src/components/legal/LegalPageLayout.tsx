...
const LegalPageLayout = ({ title, summary, children }: LegalPageLayoutProps) => {
  useEffect(() => {
    ...
    document.title = `${title} | ${LEGAL_COMPANY_NAME}`;

    return () => {
      document.title = previousTitle || LEGAL_COMPANY_NAME;
    };
  }, [title]);
    ...
    <div className="min-h-screen bg-[#F4F1EA] text-[#14171C]">
      <div className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute right-[-6rem] top-40 h-72 w-72 rounded-full bg-[#DDE6F8] blur-3xl opacity-65" />
          <div className="absolute bottom-8 left-[-4rem] h-64 w-64 rounded-full bg-white blur-3xl opacity-70" />
        </div>
        ...
        <main className="relative">
          ...
          <section className="max-w-360 mx-auto px-6 pb-18 md:pb-24">
            <article className="rounded-[32px] border border-black/10 bg-white/92 px-6 py-8 shadow-[0_24px_70px_rgba(18,21,26,0.10)] backdrop-blur-sm md:px-10 md:py-12">
