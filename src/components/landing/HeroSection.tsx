...
const HeroSection: React.FC = () => {
  ...
  const showDesktopPreview = useMediaQuery('(min-width: 768px)');
  const eyebrowMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in slide-in-from-bottom-3 duration-500';
  const titleMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100';
  const bodyMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150';
  const ctaMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200';
  const supportingMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in duration-700 delay-300';
  const listMotionClass = prefersReducedMotion
    ? ''
    : 'animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300';
  const mediaMotionClass = prefersReducedMotion
    ? ''
    ...
    <section className="relative overflow-hidden bg-white pt-28 pb-14 md:pt-32 md:pb-20">
      ...
      <div className="max-w-360 mx-auto top-12 md:top-0 px-6 relative z-10 grid lg:grid-cols-12 gap-12 lg:gap-10 items-center">
        <div className="lg:col-span-5">
          <div className={`inline-flex items-center rounded-full border border-black/10 bg-white px-4 py-1.5 text-sm text-black/70 shadow-sm ${eyebrowMotionClass}`}>
            AI workflows + live resume controls
          </div>

          <h1 className={`mt-6 text-4xl md:text-6xl lg:text-[64px] tracking-[-0.03em] leading-[1.02] text-gray-900 ${titleMotionClass}`}>
            From rough draft to
            <br />
          ...
          <p className={`mt-6 text-lg md:text-xl text-gray-600 font-light max-w-lg leading-relaxed ${bodyMotionClass}`}>
          ...
          <div className={`mt-9 flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto ${ctaMotionClass}`}>
            <a
              href="#get-started"
              className="group w-full sm:w-auto text-center bg-black text-white px-8 py-4 rounded-full text-lg font-medium border border-black transition-all duration-300 hover:bg-zinc-900 hover:-translate-y-0.5 shadow-[0_16px_36px_rgba(0,0,0,0.30)] motion-reduce:transition-none motion-reduce:transform-none"
            >
            ...
            <a
              href="#templates"
              className="w-full sm:w-auto text-center bg-white text-black border border-black/15 px-7 py-4 rounded-full text-lg font-medium transition-all duration-300 hover:bg-zinc-50 hover:-translate-y-0.5 shadow-[0_10px_22px_rgba(0,0,0,0.08)] motion-reduce:transition-none motion-reduce:transform-none"
            >
              Explore Templates
            </a>
          </div>
          <p className={`mt-3 text-sm text-gray-500 ${supportingMotionClass}`}>
            No credit card required to start.
          </p>

          <ul className={`mt-8 space-y-3 ${listMotionClass}`}>
        ...
        {showDesktopPreview ? (
          <div className="lg:col-span-7 lg:-mr-50">
            <div className={`relative hidden md:flex mx-auto w-full max-w-none lg:w-[112%] lg:translate-x-5 ${mediaMotionClass}`}>
